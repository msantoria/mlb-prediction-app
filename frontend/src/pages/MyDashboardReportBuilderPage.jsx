...
function ReportWorkspace({ open, close, objectMeta, result, fields, builderFields, generatedAt, reportDate, initialColumns, query, setQuery, reload, exportAllRows, loading, onSave, isMobile }) {
...
function MyDashboardReportBuilderContent() {
  ...
  function selectPrimaryObject(objectKey) { setActiveObject(objectKey); setResults(current => { const next = { ...current }; delete next[objectKey]; return next }); setReportOpen(false); setSaveAsOpen(false); setReportObject(objectKey); setReportResult(null); setReportColumns(defaultFieldsForObject(objectKey)); setReportQuery(defaultQueryState()); setError('') }
