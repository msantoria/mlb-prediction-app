...
import { buildReportCsv, formatEasternDateTime, mlbDateIso, safeFilenamePart } from '../lib/dashboardReportUtils.mjs'
...
import { CANONICAL_REPORT_TYPES, REPORT_BUILDER_SCHEMA_VERSION, buildExecutionDefinition, buildReportRequest, canonicalBootstrapMessage, defaultFieldsForObject, defaultReportSaveAsDraft, initialFieldsByObject, normalizeCanonicalPage, reconcileSelectedFields, reportExecutionFacts, reportFieldsForMode, savedReportExecutionMode, selectableRequestFields } from '../lib/dashboardReportBuilderState.mjs'
...
const OBJECTS = [
  ...
  { key: 'player_trends', label: 'Player Trends', description: 'User-configured hitter or pitcher movement against an authoritative rolling baseline.' },
]
...
function emptyFiltersForObject(objectKey) { return CANONICAL_REPORT_TYPES[objectKey] ? { logic: 'and', conditions: [], weights: {} } : emptyFilters() }
function cleanFilters(filters) {
  if (Array.isArray(filters?.conditions)) {
    const out = cleanCanonicalFilters(filters)
    const weights = {}
...
function formatAppliedFilter(condition, fieldMap) {
  const field = fieldMap[condition?.field]
  const operator = OPERATOR_LABELS[condition?.operator] || titleCase(condition?.operator)
  const value = operatorNeedsValue(condition?.operator)
    ? Array.isArray(condition?.value) ? condition.value.join(', ') : formatCell(condition?.value)
    : ''
  return `${field?.label || titleCase(condition?.field)} ${operator}${value ? ` ${value}` : ''}`
}
function fieldsForResult(result, objectKey) {
  if (CANONICAL_REPORT_TYPES[objectKey] && !result?.object_info?.fields?.length) return []
  ...
  if (CANONICAL_REPORT_TYPES[objectKey] && described.length) return described
  ...
  return Array.from(seen.values()).filter(field => field.selectable !== false)
}
...
const OPERATOR_LABELS = {
  eq: 'Equals', neq: 'Does not equal', contains: 'Contains', in: 'Is one of',
  gt: 'Greater than', gte: 'At least', lt: 'Less than', lte: 'At most',
  is_null: 'Is blank', is_not_null: 'Is not blank',
}
function CanonicalFilterPanel({ objectKey, filters, fields, setBasic, setWeight }) {
  ...
  return <section style={s.card}>
    <div style={s.cardHeader}><div><div style={s.panelTitle}>Filters</div><div style={s.copySmall}>Build typed conditions from every available report field.</div></div><Pill>{conditions.length} conditions</Pill></div>
    <div style={s.logicBar}><span style={s.logicLabel}>Filter logic</span>{FILTER_LOGIC_OPTIONS.map(option => <button type="button" key={option.value} aria-pressed={(filters.logic || 'and') === option.value} style={(filters.logic || 'and') === option.value ? s.logicActive : s.logicButton} onClick={() => setBasic(objectKey, 'logic', option.value)}>{option.label}</button>)}{conditions.length ? <button type="button" style={s.clearButton} onClick={() => replaceConditions([])}>Clear</button> : null}</div>
    <div style={s.conditionList}>{conditions.map((condition, index) => {
      const field = available.find(item => item.accessor === condition.field)
      const orphaned = !field
      const operators = field?.supportedOperators?.length ? field.supportedOperators : [condition.operator || 'eq']
      const operator = orphaned ? (condition.operator || 'eq') : operators.includes(condition.operator) ? condition.operator : defaultOperator(field)
      ...
      return <div style={s.conditionCard} key={`${condition.field}-${index}`}>
        ...
        <label style={s.filterControl}><span>Field</span><select style={s.input} value={condition.field || ''} onChange={event => {
          const nextField = available.find(item => item.accessor === event.target.value)
          updateCondition(index, { field: event.target.value, operator: defaultOperator(nextField), value: '' })
        }}>{orphaned ? <option value={condition.field}>Unavailable field: {condition.field}</option> : null}{Object.entries(grouped).map(([group, groupFields]) => <optgroup key={group} label={group}>{groupFields.map(item => <option key={item.accessor} value={item.accessor}>{item.label}</option>)}</optgroup>)}</select>{orphaned ? <small style={s.orphanedFilter}>This field is no longer registered. Select a replacement or remove the condition.</small> : null}</label>
        <label style={s.filterControl}><span>Operator</span><select disabled={orphaned} style={s.input} value={operator} onChange={event => updateCondition(index, { operator: event.target.value, value: '' })}>{operators.map(value => <option key={value} value={value}>{OPERATOR_LABELS[value] || titleCase(value)}</option>)}</select></label>
        <label style={s.filterControl}><span>Value</span>{!needsValue ? <span style={s.valueNotRequired}>No value required</span> : confidenceValues && operator !== 'contains' && operator !== 'in' ? <select disabled={orphaned} style={s.input} value={condition.value ?? ''} onChange={event => updateCondition(index, { value: event.target.value })}><option value="">Choose confidence</option><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option></select> : <input disabled={orphaned} style={s.input} type={filterInputType(field, operator)} step={field?.dataType === 'integer' || field?.dataType === 'id' ? '1' : 'any'} placeholder={operator === 'in' ? 'Comma-separated values' : `Enter ${field?.label || 'value'}`} value={condition.value ?? ''} onChange={event => updateCondition(index, { value: event.target.value })} />}</label>
        <button type="button" aria-label={`Remove filter ${index + 1}`} title="Remove condition" style={s.removeCondition} onClick={() => replaceConditions(conditions.filter((_condition, conditionIndex) => conditionIndex !== index))}>×</button>
      </div>
    })}</div>
...
function PlayerTrendsConfiguration({ value, onChange }) {
  const config = { ...EMPTY_TREND_CONFIG, ...(value || {}) }; const metrics = TREND_METRICS[config.player_type] || []; const update = patch => onChange({ ...config, ...patch })
  return <section style={s.card}><div style={s.panelTitle}>Required Trend Configuration</div><div style={s.copySmall}>Choose what “trending” means before this report can run. The calculation reuses the batter and pitcher rolling-page authorities.</div><div style={s.filterGrid}>
    <label style={s.filterControl}><span>Player type</span><select style={s.input} value={config.player_type} onChange={event => update({ player_type: event.target.value, selected_metrics: [] })}><option value="">Choose hitters or pitchers</option><option value="hitter">Hitters</option><option value="pitcher">Pitchers</option></select></label>
    <label style={s.filterControl}><span>N-day window</span><input style={s.input} type="number" min="3" max="90" list="trend-window-presets" value={config.window_days} onChange={event => update({ window_days: event.target.value })} placeholder="7, 15, 30, 60, or custom" /><datalist id="trend-window-presets"><option value="7" /><option value="15" /><option value="30" /><option value="60" /></datalist></label>
    <label style={s.filterControl}><span>Comparison baseline</span><select style={s.input} value={config.comparison_baseline} onChange={event => update({ comparison_baseline: event.target.value })}><option value="">Choose a baseline</option><option value="season_to_date">Season to date</option><option value="previous_n_days">Previous N days</option></select></label>
    <label style={s.filterControl}><span>{config.player_type === 'pitcher' ? 'Minimum batters faced' : 'Minimum plate appearances'}</span><input style={s.input} type="number" min="1" max="1000" value={config.minimum_sample_size} onChange={event => update({ minimum_sample_size: event.target.value })} placeholder="Required" /></label>
    <label style={s.filterControl}><span>Trend direction</span><select style={s.input} value={config.trend_direction} onChange={event => update({ trend_direction: event.target.value })}><option value="all">All</option><option value="improving">Improving</option><option value="declining">Declining</option><option value="stable">Stable</option></select></label>
  </div><div style={s.sectionLabel}>Trend metrics</div>{config.player_type ? <div style={s.metricGrid}>{metrics.map(([metric, label]) => <label key={metric} style={s.metricCard}><span><input type="checkbox" checked={config.selected_metrics.includes(metric)} onChange={() => update({ selected_metrics: config.selected_metrics.includes(metric) ? config.selected_metrics.filter(value => value !== metric) : [...config.selected_metrics, metric] })} /> {label}</span></label>)}</div> : <StatePanel title="Choose a player type">The supported rolling metrics will appear here.</StatePanel>}</section>
}
function FieldLibrary({ fields, selected, setSelected }) {
  const selectableFields = fields.filter(field => field.selectable !== false)
  const selectedSet = new Set(selected); const grouped = selectableFields.reduce((map, field) => ({ ...map, [field.group]: [...(map[field.group] || []), field] }), {})
...
function SaveAsDialog({ open, folders, draft, setDraft, saving, onSave, onClose }) {
  if (!open) return null
  const selectedFolder = folders.find(folder => String(folder.id) === String(draft.folder_id))
  return <div style={s.saveOverlay} role="dialog" aria-modal="true" aria-labelledby="save-as-title">
    <section style={s.saveSurface}>
      <div style={s.cardHeader}>
        <div><div style={s.eyebrow}>MyDashboard</div><h2 id="save-as-title" style={s.reportTitle}>Save report as</h2><div style={s.copySmall}>Name this report and choose exactly where it should be saved.</div></div>
        <button type="button" style={s.secondary} onClick={onClose} disabled={saving}>Cancel</button>
      </div>
      <label style={s.saveLabel}>Report name<input autoFocus maxLength={255} style={s.input} value={draft.title} onChange={event => setDraft(current => ({ ...current, title: event.target.value }))} placeholder="Report name" /></label>
      <div>
        <div style={s.sectionLabel}>Folder selection</div>
        <div style={s.copySmall}>Choose a daily, default, or custom MyDashboard folder.</div>
        <div style={s.saveFolderGrid}>{folders.map(folder => {
          const selected = String(folder.id) === String(draft.folder_id)
          const detail = folder.folder_date || (folder.is_default ? 'Default folder' : 'Custom folder')
          return <button type="button" key={folder.id} aria-pressed={selected} style={selected ? s.saveFolderActive : s.saveFolder} onClick={() => setDraft(current => ({ ...current, folder_id: String(folder.id) }))}><span><strong>📁 {folder.folder_name}</strong><small>{detail}</small></span><b>{folder.item_count || 0}</b></button>
        })}</div>
      </div>
      {!folders.length ? <StatePanel tone="error" title="No folders available">Create a folder from the Saved Reports section, then try again.</StatePanel> : null}
      <div style={s.saveFooter}><span style={s.copySmall}>{selectedFolder ? `Saving to ${selectedFolder.folder_name}` : 'Choose a folder to continue.'}</span><button type="button" style={s.primary} disabled={saving || !draft.title.trim() || !selectedFolder} onClick={onSave}>{saving ? 'Saving…' : 'Save Report'}</button></div>
    </section>
  </div>
}
...
function ReportWorkspace({ open, close, objectMeta, result, fields, builderFields, generatedAt, reportDate, initialColumns, query, setQuery, reload, loading, onSave, isMobile }) {
  ...
  const execution = reportExecutionFacts(result)
  const executionDefinition = result?.execution_definition || {}
  const appliedFilters = Array.isArray(executionDefinition?.filters?.conditions)
    ? executionDefinition.filters.conditions
    : safeArray(result?.filters_applied)
  const appliedLogic = executionDefinition?.filters?.logic || result?.filter_logic || 'and'
  if (!open) return null
  ...
  return <div style={s.overlay} role="dialog" aria-modal="true"><section style={s.reportSurface}><header style={s.reportHeader}><div><div style={s.eyebrow}>Report Workspace</div><h2 style={s.reportTitle}>{objectMeta.label} Report</h2><div style={s.copySmall}>MLB date {reportDate} · {execution.mode} · Projection {execution.snapshotDate || 'date unavailable'} · Generated {generatedAt ? formatEasternDateTime(generatedAt) : 'now'} · Showing {range.start}–{range.end} of {range.total}</div></div><div style={s.actions}><button style={s.secondary} onClick={exportCsv} disabled={!rows.length}>Export current page CSV</button><button style={s.secondary} onClick={onSave} disabled={!rows.length}>Save As</button><button style={s.primary} onClick={close}>Back to Builder</button></div></header><div style={{ ...s.reportBody, ...(isMobile ? s.reportBodyMobile : {}) }}><aside style={s.columnPanel}><div style={s.panelTitle}>Report Columns</div><div style={s.copySmall}>Hide or reorder columns without changing saved results.</div><div style={s.columnList}>{columns.map((accessor, index) => { const isHidden = hidden.includes(accessor); return <div style={s.columnRow} key={accessor}><label><input type="checkbox" checked={!isHidden} onChange={() => setHidden(current => isHidden ? current.filter(v => v !== accessor) : [...current, accessor])} /> {fieldMap[accessor]?.label || titleCase(accessor)}</label><div><button style={s.iconButton} disabled={index === 0} onClick={() => setColumns(current => { const next = [...current]; [next[index - 1], next[index]] = [next[index], next[index - 1]]; return next })}>↑</button><button style={s.iconButton} disabled={index === columns.length - 1} onClick={() => setColumns(current => { const next = [...current]; [next[index + 1], next[index]] = [next[index], next[index + 1]]; return next })}>↓</button></div></div> })}</div><button style={s.secondaryWide} onClick={() => { setColumns(builderFields); setHidden([]) }}>Reset to Executed Fields</button><div style={s.sectionLabel}>Applied Filters</div>{appliedFilters.length ? <div style={s.appliedFilters}><strong>{appliedLogic === 'or' ? 'Match any' : 'Match all'}</strong>{appliedFilters.map((condition, index) => <span key={`${condition.field}-${condition.operator}-${index}`}>{formatAppliedFilter(condition, fieldMap)}</span>)}</div> : <div style={s.copySmall}>No filters were applied to this execution.</div>}</aside><main style={s.gridPanel}><div style={s.gridToolbar}><div style={s.pillRow}><Pill>{range.total} matching rows</Pill><Pill>{execution.mode}</Pill>{execution.lineupStatus ? <Pill tone="green">Lineups: {execution.lineupStatus}{execution.confirmedCount != null ? ` · ${execution.confirmedCount} confirmed` : ''}</Pill> : null}<Pill tone="green">Sorted by: {fieldMap[query.sort_by]?.label || titleCase(query.sort_by)} {query.sort_direction}</Pill></div><label style={s.pageSize}>Rows per page <select style={s.input} value={query.page_size} onChange={e => requestPageSize(e.target.value)}>{PAGE_SIZE_OPTIONS.map(size => <option key={size} value={size}>{size}</option>)}</select></label></div>{execution.ageHours != null && execution.ageHours > 36 ? <div style={s.warning}>Projection freshness warning: {execution.ageHours} hours old.</div> : null}{result?.filter_warnings?.length ? <div style={s.warning}>{result.filter_warnings.join(' • ')}</div> : null}{loading ? <StatePanel tone="loading" title="Loading report page">Loading the complete report.</StatePanel> : rows.length && visible.length ? <div style={s.dataGridWrap}><table style={s.table}><thead><tr>{visible.map(accessor => <th key={accessor} style={s.th}><button style={s.sortButton} disabled={fieldMap[accessor]?.sortable === false} onClick={() => requestSort(accessor)}>{fieldMap[accessor]?.label || titleCase(accessor)} {query.sort_by === accessor ? (query.sort_direction === 'desc' ? '↓' : '↑') : '↕'}</button></th>)}</tr></thead><tbody>{rows.map((row, rowIndex) => <tr key={row.entity_id || `${objectMeta.key}-${rowIndex}`}>{visible.map(accessor => <td style={s.td} key={`${rowIndex}-${accessor}`}>{formatCell(getValue(row, accessor))}</td>)}</tr>)}</tbody></table></div> : <StatePanel tone={bootstrapMessage.tone} title={bootstrapMessage.title}>{bootstrapMessage.detail}</StatePanel>}<div style={s.pagination}><button style={s.secondary} disabled={!pageInfo.has_previous || loading} onClick={() => requestPage(pageInfo.previous_page || Math.max(1, query.page_number - 1))}>← Previous</button><span>Page {pageInfo.page_number || 1} of {pageInfo.page_count || 1}</span><button style={s.secondary} disabled={!pageInfo.has_next || loading} onClick={() => requestPage(pageInfo.next_page || query.page_number + 1)}>Next →</button></div></main></div></section></div>
}
...
function MyDashboardReportBuilderContent() {
  ...
  const [trendConfig, setTrendConfig] = useState(() => ({ ...EMPTY_TREND_CONFIG, ...(persisted.trendConfig || {}) }))
  const selectedFields = selectedFieldsByObject[activeObject] || defaultFieldsForObject(activeObject)
  const setSelectedFields = next => {
    invalidateGeneratedReport(activeObject)
    setSelectedFieldsByObject(current => ({ ...current, [activeObject]: typeof next === 'function' ? next(current[activeObject] || defaultFieldsForObject(activeObject)) : next }))
  }
  const activeMeta = OBJECTS.find(object => object.key === activeObject) || OBJECTS[0]; const activeResult = results[activeObject]; const activeReportType = CANONICAL_REPORT_TYPES[activeObject]; const activeCatalog = reportTypes.find(reportType => reportType.api_name === activeReportType); const activeMetadataResult = activeResult || (activeCatalog ? { object_info: activeCatalog } : null); const activeFields = useMemo(() => fieldsForResult(activeMetadataResult, activeObject), [activeMetadataResult, activeObject]); const activeFilters = filters[activeObject] || emptyFiltersForObject(activeObject); const folderId = Number(selectedFolderId || workspace?.today_folder_id || safeArray(workspace?.folders)[0]?.id)
  useEffect(() => {
    if (!activeFields.length) return
    setSelectedFieldsByObject(current => {
      const reconciled = reconcileSelectedFields(current[activeObject], activeFields, defaultFieldsForObject(activeObject))
      if (reconciled.length === (current[activeObject] || []).length && reconciled.every((field, index) => field === current[activeObject][index])) return current
      return { ...current, [activeObject]: reconciled }
    })
  }, [activeFields, activeObject])
  useEffect(() => { writeJson(BUILDER_KEY, { schema_version: REPORT_BUILDER_SCHEMA_VERSION, activeObject, reportDate, filters, selectedFieldsByObject, activeLineupsOnly, trendConfig }) }, [activeObject, reportDate, filters, selectedFieldsByObject, activeLineupsOnly, trendConfig])
  ...
  function setBasic(objectKey, key, value) { invalidateGeneratedReport(objectKey); setFilters(current => ({ ...current, [objectKey]: { ...(current[objectKey] || emptyFiltersForObject(objectKey)), [key]: value } })) }
  function setMetric(objectKey, metric, side, value) { invalidateGeneratedReport(objectKey); setFilters(current => ({ ...current, [objectKey]: { ...(current[objectKey] || emptyFilters()), metrics: { ...(current[objectKey]?.metrics || {}), [metric]: { ...(current[objectKey]?.metrics?.[metric] || {}), [side]: value } } } })) }
  function setWeight(objectKey, metric, value) { invalidateGeneratedReport(objectKey); setFilters(current => ({ ...current, [objectKey]: { ...(current[objectKey] || emptyFiltersForObject(objectKey)), weights: { ...(current[objectKey]?.weights || {}), [metric]: value } } })) }
  function invalidateGeneratedReport(objectKey) { setResults(current => { const next = { ...current }; delete next[objectKey]; return next }); if (reportObject === objectKey) { setReportOpen(false); setSaveAsOpen(false); setReportResult(null); setGeneratedAt(null) }; setError('') }
  function invalidateGeneratedReports() { setResults({}); setReportOpen(false); setSaveAsOpen(false); setReportResult(null); setGeneratedAt(null); setError('') }
  function changeReportDate(value) { const next = value || mlbDateIso(); if (next !== reportDate) invalidateGeneratedReports(); setReportDate(next) }
  function changeLineupMode(value) { const next = Boolean(value); if (next !== activeLineupsOnly) invalidateGeneratedReports(); setActiveLineupsOnly(next) }
  function selectPrimaryObject(objectKey) { setActiveObject(objectKey); setReportOpen(false); setSaveAsOpen(false); setReportObject(objectKey); setReportResult(null); setReportColumns(defaultFieldsForObject(objectKey)); setReportQuery(defaultQueryState()); setError('') }
  async function populateReport(objectKey = activeObject, queryOverride = null, openWorkspace = true) {
    if (objectKey === 'player_trends') {
      const days = Number(trendConfig.window_days), minimum = Number(trendConfig.minimum_sample_size)
      if (!trendConfig.player_type || !trendConfig.comparison_baseline || !Number.isInteger(days) || days < 3 || days > 90 || !Number.isInteger(minimum) || minimum < 1 || !trendConfig.selected_metrics.length) {
        setError('Choose a player type, valid 3–90 day window, comparison baseline, minimum sample, and at least one trend metric.')
        return null
      }
    }
    const query = normalizeQueryState(queryOverride || (objectKey === reportObject ? reportQuery : defaultQueryState()))
    const cleaned = cleanFilters(filters[objectKey] || {})
    const objectFields = reportFieldsForMode({ objectKey, activeLineupsOnly, selectedFields: selectedFieldsByObject[objectKey] })
    const availableFields = objectKey === activeObject ? activeFields : fieldsForResult(results[objectKey], objectKey)
    if (CANONICAL_REPORT_TYPES[objectKey] && !availableFields.length) {
      setError('The registered field catalog is still loading or unavailable. Reload the workspace before populating this report.')
      return null
    }
    const availableFilterNames = new Set(filterableReportFields(availableFields).map(field => field.accessor))
    const orphanedFilters = safeArray(cleaned.conditions).filter(condition => !availableFilterNames.has(condition.field))
    if (orphanedFilters.length) {
      setError(`Remove or replace unavailable filter fields: ${orphanedFilters.map(condition => condition.field).join(', ')}.`)
      return null
    }
    const requestedFields = selectableRequestFields(objectFields, availableFields)
    const normalizedTrendConfig = objectKey === 'player_trends'
      ? { ...trendConfig, window_days: Number(trendConfig.window_days), minimum_sample_size: Number(trendConfig.minimum_sample_size) }
      : undefined
    const request = buildReportRequest({ objectKey, activeLineupsOnly, date: reportDate, cleanedFilters: cleaned, query, selectedFields: requestedFields, trendConfig: normalizedTrendConfig })
    const reportType = request.reportType
    const payload = request.payload
    setLoading(current => ({ ...current, [objectKey]: true }))
    setError('')
    setSaveMessage('')
    try {
      const raw = await apiJson(request.path, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
      const normalized = reportType ? normalizeCanonicalPage(raw, query) : raw
      const generatedAtValue = normalized?.provenance?.generated_at || new Date().toISOString()
      const executionDefinition = buildExecutionDefinition({ objectKey, reportType, result: normalized, requestPayload: payload, query, generatedForDate: reportDate, generatedAt: generatedAtValue, displayFields: objectFields, trendConfig: normalizedTrendConfig })
      const json = { ...normalized, execution_definition: executionDefinition }
      const executedQuery = normalizeQueryState({ page_number: json?.page_info?.page_number || query.page_number, page_size: executionDefinition.page_size, sort_by: executionDefinition.sort.by, sort_direction: executionDefinition.sort.direction })
      setResults(current => ({ ...current, [objectKey]: json }))
      setReportObject(objectKey)
      setReportResult(json)
      setReportColumns([...executionDefinition.selected_fields])
      setReportQuery(executedQuery)
      setGeneratedAt(generatedAtValue)
      setGeneratedForDate(executionDefinition.as_of_date || reportDate)
      if (openWorkspace) setReportOpen(true)
      return json
    } catch (err) {
      setError(err.message || 'Report generation failed')
      return null
    } finally {
      setLoading(current => ({ ...current, [objectKey]: false }))
    }
  }
  function openLastReport(objectKey = activeObject) {
    const result = results[objectKey]
    if (!result) return
    const definition = result.execution_definition || {}
    const columns = definition.selected_fields || selectedFieldsByObject[objectKey] || defaultFieldsForObject(objectKey)
    setReportObject(objectKey)
    setReportResult(result)
    setReportColumns([...columns])
    setReportQuery(normalizeQueryState({ page_number: result?.page_info?.page_number || 1, page_size: definition.page_size || result?.page_info?.page_size, sort_by: definition.sort?.by || result?.query?.sort_by, sort_direction: definition.sort?.direction || result?.query?.sort_direction }))
    setGeneratedForDate(definition.as_of_date || reportDate)
    setGeneratedAt(definition.generated_at || result?.provenance?.generated_at || new Date().toISOString())
    setReportOpen(true)
  }
  function openSaved(item) {
    ...
    const board = snapshot.board_state || payload.board_state || payload.report_state || payload
    const component = definition.component || payload.saved_from_component || item?.sort_json?.component || activeObject
    const executionDefinition = board?.execution_definition || definition
    const columns = executionDefinition.selected_fields || payload.workbench_state?.selectedFields || payload.report_columns || selectedFieldsByObject[component] || defaultFieldsForObject(component)
    const sort = executionDefinition.sort || item?.sort_json || defaultQueryState()
    const savedDate = executionDefinition.as_of_date || snapshot.generated_for_date || payload.saved_on_date || reportDate
    const savedLineupMode = savedReportExecutionMode(item, activeLineupsOnly)
    if (definition.query_studio_statement) setQueryStudioRestore({ statement: definition.query_studio_statement, restoredAt: Date.now() })
    if (!board?.items && !board?.records) { setSaveMessage('This saved item does not contain report rows.'); return }
    setActiveObject(component)
    setReportObject(component)
    setReportDate(savedDate)
    setActiveLineupsOnly(savedLineupMode)
    setFilters(current => ({ ...current, [component]: CANONICAL_REPORT_TYPES[component] ? normalizeSavedFilters(executionDefinition.filters || {}) : executionDefinition.filters || current[component] || emptyFilters() }))
    if (component === 'player_trends' && executionDefinition.trend_config) setTrendConfig({ ...EMPTY_TREND_CONFIG, ...executionDefinition.trend_config })
    setSelectedFieldsByObject(current => ({ ...current, [component]: [...columns] }))
    const restoredBoard = { ...board, execution_definition: executionDefinition }
    setResults(current => ({ ...current, [component]: restoredBoard }))
    setReportResult(restoredBoard)
    setReportColumns(columns)
    setReportQuery(normalizeQueryState({ page_number: board?.page_info?.page_number || 1, page_size: board?.page_info?.page_size || DEFAULT_PAGE_SIZE, sort_by: sort.by || sort.sort_by || 'score', sort_direction: sort.direction || sort.sort_direction || 'desc' }))
    setGeneratedForDate(savedDate)
    setGeneratedAt(executionDefinition.generated_at || snapshot.generated_at || item.created_at || new Date().toISOString())
    setReportOpen(true)
  }
  function saveReport() {
    const rows = safeArray(reportResult?.records?.length ? reportResult.records : reportResult?.items)
    if (!rows.length) { setSaveMessage('Populate a report before saving.'); return }
    const folders = safeArray(workspace?.folders)
    if (!folders.length) { setSaveMessage('No dashboard folder is available.'); return }
    const meta = OBJECTS.find(object => object.key === reportObject) || activeMeta
    const preferredFolderId = selectedFolderId || workspace?.today_folder_id || folders.find(folder => folder.is_default)?.id || folders[0].id
    setSaveAsDraft(defaultReportSaveAsDraft({ label: meta.label, date: generatedForDate, folderId: preferredFolderId }))
    setSaveMessage('')
    setSaveAsOpen(true)
  }
  async function commitSaveReport() {
    const rows = safeArray(reportResult?.records?.length ? reportResult.records : reportResult?.items)
    const title = saveAsDraft.title.trim()
    const destination = safeArray(workspace?.folders).find(folder => String(folder.id) === String(saveAsDraft.folder_id))
    if (!rows.length) { setSaveMessage('Populate a report before saving.'); return }
    if (!title) { setSaveMessage('Name the report before saving.'); return }
    if (!destination) { setSaveMessage('Choose a destination folder before saving.'); return }
    const meta = OBJECTS.find(object => object.key === reportObject) || activeMeta
    const frozenDefinition = reportResult?.execution_definition
    const definition = frozenDefinition
      ? { ...frozenDefinition, selected_fields: [...reportColumns] }
      : { component: reportObject, report_type: CANONICAL_REPORT_TYPES[reportObject] || null, selected_fields: reportColumns, filters: cleanFilters(filters[reportObject] || {}), active_lineups_only: activeLineupsOnly, sort: { by: reportQuery.sort_by, direction: reportQuery.sort_direction }, page_size: reportQuery.page_size, trend_config: reportObject === 'player_trends' ? { ...trendConfig, window_days: Number(trendConfig.window_days), minimum_sample_size: Number(trendConfig.minimum_sample_size) } : undefined }
    const snapshot = { generated_for_date: generatedForDate, generated_at: generatedAt, board_state: reportResult }
    setSavingReport(true)
    setSaveMessage('')
    try {
      await apiJson('/my-dashboard/items', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ folder_id: destination.id, source_tab: 'my-dashboard', source_type: 'report_view', title, subtitle: meta.description, notes: 'Saved from the MLBGPT Report Workspace.', payload_json: { schema_version: 4, definition, snapshot, saved_from_component: reportObject, saved_on_date: generatedForDate, board_state: reportResult, report_columns: reportColumns, workbench_state: { selectedFields: reportColumns, activeLineupsOnly: definition.active_lineups_only } }, filter_json: definition.filters, sort_json: { ...definition.sort, component: reportObject, report_workspace: true } }) })
      await loadWorkspace()
      const destinationId = String(destination.id)
      const shelfView = destination.folder_date ? 'daily' : 'custom'
      setSelectedFolderId(destinationId)
      setSavedShelfOpen(true)
      setSavedShelfView(shelfView)
      setSelectedShelfEntryKey(`${shelfView}:${destinationId}`)
      setSaveAsOpen(false)
      setSaveMessage(`Saved ${title} to ${destination.folder_name}.`)
    } catch (err) {
      setSaveMessage(err.message || 'Failed to save report.')
    } finally {
...
const s = {
  ...
  saveSurface: { width: 'min(100%,680px)', maxHeight: 'min(760px,calc(100vh - 32px))', display: 'grid', gap: 18, overflowY: 'auto', boxSizing: 'border-box', padding: 'clamp(18px,4vw,30px)', background: 'var(--md-report-bg)', border: '1px solid var(--md-glass-border)', borderRadius: 24, boxShadow: '0 24px 70px rgba(0,0,0,.4)' },
  saveLabel: { display: 'grid', gap: 7, color: C.muted, fontFamily: FRANKLIN, fontSize: 11 },
  saveFolderGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 8, marginTop: 10 },
  saveFolder: { display: 'flex', justifyContent: 'space-between', gap: 9, padding: 11, textAlign: 'left', color: C.text, fontFamily: CENTURY, background: C.panel2, border: `1px solid ${C.border}`, borderRadius: 11 },
  saveFolderActive: { display: 'flex', justifyContent: 'space-between', gap: 9, padding: 11, textAlign: 'left', color: C.text, fontFamily: CENTURY, background: 'rgba(112,100,245,.16)', border: '1px solid rgba(112,100,245,.55)', borderRadius: 11, boxShadow: '0 8px 20px rgba(86,72,206,.12)' },
  saveFooter: { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', flexWrap: 'wrap', paddingTop: 4 },
  ...
  logicBar: { display: 'flex', gap: 7, alignItems: 'center', flexWrap: 'wrap', marginTop: 11 },
  logicLabel: { marginRight: 2, color: C.muted, fontFamily: FRANKLIN, fontSize: 10, fontWeight: 500, letterSpacing: '.06em', textTransform: 'uppercase' },
  logicButton: { padding: '7px 10px', color: C.text, fontFamily: FRANKLIN, fontSize: 11, background: 'var(--md-control-soft)', border: `1px solid ${C.border}`, borderRadius: 999 },
  logicActive: { padding: '7px 10px', color: '#fff', fontFamily: FRANKLIN, fontSize: 11, background: 'linear-gradient(105deg,#2b2e36,#776df5)', border: '1px solid rgba(255,255,255,.5)', borderRadius: 999 },
  clearButton: { marginLeft: 'auto', padding: '7px 10px', color: C.muted, fontFamily: FRANKLIN, fontSize: 11, background: 'transparent', border: `1px solid ${C.border}`, borderRadius: 999 },
  conditionList: { display: 'grid', gap: 8, marginTop: 9 },
  conditionCard: { position: 'relative', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(135px,1fr))', gap: 7, alignItems: 'end', padding: '9px 9px 9px 38px', background: C.panel2, border: `1px solid ${C.border}`, borderRadius: 11 },
  conditionNumber: { position: 'absolute', top: 17, left: 13, color: C.blue, fontFamily: FRANKLIN, fontSize: 11, fontWeight: 500 },
  orphanedFilter: { color: C.red, fontSize: 10, lineHeight: 1.35 },
  appliedFilters: { display: 'grid', gap: 6, padding: 9, color: C.muted, fontSize: 10, background: C.panel2, border: `1px solid ${C.border}`, borderRadius: 9 },
  filterControl: { display: 'grid', gap: 4, minWidth: 0, color: C.muted, fontSize: 10 },
  valueNotRequired: { display: 'grid', alignItems: 'center', minHeight: 34, padding: '0 9px', color: C.muted, fontSize: 10, background: 'var(--md-control-soft)', border: `1px dashed ${C.border}`, borderRadius: 8 },
  removeCondition: { padding: '8px 9px', color: C.red, fontFamily: FRANKLIN, fontSize: 11, background: 'rgba(196,74,86,.08)', border: '1px solid rgba(196,74,86,.25)', borderRadius: 8 },
  addCondition: { justifySelf: 'start', marginTop: 9, padding: '8px 11px', color: C.blue, fontFamily: FRANKLIN, fontSize: 11, background: 'rgba(112,100,245,.1)', border: '1px solid rgba(112,100,245,.35)', borderRadius: 999 },
