...
import { formatEasternDateTime, mlbDateIso, safeFilenamePart } from '../lib/dashboardReportUtils.mjs'
...
function fieldsForResult(result, objectKey) {
  ...
  if (CANONICAL_REPORT_TYPES[objectKey] && described.length) {
    return described.filter(field => field.selectable !== false)
  }
...
function CanonicalFilterPanel({ objectKey, filters, fields, setBasic, setWeight }) {
  ...
  return <section style={s.card}>
    ...
    <div style={s.conditionList}>{conditions.map((condition, index) => {
      const field = available.find(item => item.accessor === condition.field) || available[0]
      ...
      return <div style={s.conditionCard} key={`${condition.field}-${index}`}>
        ...
        }}>{Object.entries(grouped).map(([group, groupFields]) => <optgroup key={group} label={group}>{groupFields.map(item => <option key={item.accessor} value={item.accessor}>{item.label}</option>)}</optgroup>)}</select></label>
        <label style={s.filterControl}><span>Operator</span><select style={s.input} value={operator} onChange={event => updateCondition(index, { operator: event.target.value, value: '' })}>{operators.map(value => <option key={value} value={value}>{OPERATOR_LABELS[value] || titleCase(value)}</option>)}</select></label>
        <label style={s.filterControl}><span>Value</span>{!needsValue ? <span style={s.valueNotRequired}>No value required</span> : confidenceValues && operator !== 'contains' && operator !== 'in' ? <select style={s.input} value={condition.value ?? ''} onChange={event => updateCondition(index, { value: event.target.value })}><option value="">Choose confidence</option><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option></select> : <input style={s.input} type={filterInputType(field, operator)} step={field?.dataType === 'integer' || field?.dataType === 'id' ? '1' : 'any'} placeholder={operator === 'in' ? 'Comma-separated values' : `Enter ${field?.label || 'value'}`} value={condition.value ?? ''} onChange={event => updateCondition(index, { value: event.target.value })} />}</label>
...
function ReportWorkspace({ open, close, objectMeta, result, fields, builderFields, generatedAt, reportDate, initialColumns, query, setQuery, reload, exportAllRows, loading, onSave, savedReportId, isMobile }) {
  ...
  const [subscription, setSubscription] = useState(null); const [subscriptionLoading, setSubscriptionLoading] = useState(false); const [subscriptionError, setSubscriptionError] = useState('')
  useEffect(() => { setColumns(initialColumns); setHidden([]) }, [initialColumns, result])
  useEffect(() => { setExportError('') }, [objectMeta.key, reportDate, query.sort_by, query.sort_direction])
  useEffect(() => {
    if (!open || !savedReportId) { setSubscription(null); setSubscriptionError(''); setSubscriptionLoading(false); return undefined }
    let cancelled = false
    setSubscriptionLoading(true); setSubscriptionError('')
    dashboardApi(`/my-dashboard/items/${savedReportId}/subscription`)
      .then(json => { if (!cancelled) { setSubscription(json); setSubscriptionError(json.last_error || '') } })
      .catch(err => { if (!cancelled) setSubscriptionError(err.message || 'Unable to load report subscription.') })
  ...
  async function exportCsv() {
    if (!rows.length || !visible.length || exporting) return
    setExporting(true)
    setExportError('')
    try {
      const response = await exportAllRows(visible)
      await downloadResponseFile(response, `${safeFilenamePart(objectMeta.label)}-${reportDate}-all-rows.csv`)
    } catch (err) {
      setExportError(err.message || 'Unable to export every matching report row.')
    } finally {
      setExporting(false)
    }
  }
  async function toggleSubscription() {
    if (!savedReportId) { onSave(); return }
    if (subscriptionLoading) return
    setSubscriptionLoading(true); setSubscriptionError('')
    try {
      const json = await dashboardApi(`/my-dashboard/items/${savedReportId}/subscription`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ enabled: !subscription?.enabled }) })
      setSubscription(json)
      setSubscriptionError(json.last_error || '')
    } catch (err) {
      setSubscriptionError(err.message || 'Unable to update report subscription.')
    } finally {
      setSubscriptionLoading(false)
    }
  }
  const subscriptionLabel = !savedReportId ? 'Save report to subscribe' : subscriptionLoading ? 'Updating subscription…' : subscription?.enabled ? 'Subscribed ✓' : 'Subscribe to report'
  return <div style={s.overlay} role="dialog" aria-modal="true"><section style={s.reportSurface}><header style={s.reportHeader}><div><div style={s.eyebrow}>Report Workspace</div><h2 style={s.reportTitle}>{objectMeta.label} Report</h2><div style={s.copySmall}>MLB date {reportDate} · {execution.mode} · Projection {execution.snapshotDate || 'date unavailable'} · Generated {generatedAt ? formatEasternDateTime(generatedAt) : 'now'} · Showing {range.start}–{range.end} of {range.total}</div>{exportError ? <div style={s.exportError}>{exportError}</div> : null}{subscriptionError ? <div style={s.exportError}>{subscriptionError}</div> : null}</div><div style={s.actions}><button style={subscription?.enabled ? s.primary : s.secondary} aria-pressed={Boolean(subscription?.enabled)} onClick={toggleSubscription} disabled={!rows.length || loading || exporting || subscriptionLoading} title={subscription?.enabled ? 'Click to unsubscribe' : undefined}>{subscriptionLabel}</button><button style={s.secondary} onClick={exportCsv} disabled={!rows.length || loading || exporting}>{exporting ? 'Exporting all rows…' : 'Export all rows CSV'}</button><button style={s.secondary} onClick={onSave} disabled={!rows.length || exporting}>Save As</button><button style={s.primary} onClick={close}>Back to Builder</button></div></header><div style={{ ...s.reportBody, ...(isMobile ? s.reportBodyMobile : {}) }}><aside style={s.columnPanel}><div style={s.panelTitle}>Report Columns</div><div style={s.copySmall}>Hide or reorder columns without changing saved results.</div><div style={s.columnList}>{columns.map((accessor, index) => { const isHidden = hidden.includes(accessor); return <div style={s.columnRow} key={accessor}><label><input type="checkbox" checked={!isHidden} onChange={() => setHidden(current => isHidden ? current.filter(v => v !== accessor) : [...current, accessor])} /> {fieldMap[accessor]?.label || titleCase(accessor)}</label><div><button style={s.iconButton} disabled={index === 0} onClick={() => setColumns(current => { const next = [...current]; [next[index - 1], next[index]] = [next[index], next[index - 1]]; return next })}>↑</button><button style={s.iconButton} disabled={index === columns.length - 1} onClick={() => setColumns(current => { const next = [...current]; [next[index + 1], next[index]] = [next[index], next[index + 1]]; return next })}>↓</button></div></div> })}</div><button style={s.secondaryWide} onClick={() => { setColumns(builderFields); setHidden([]) }}>Reset to Builder Fields</button></aside><main style={s.gridPanel}><div style={s.gridToolbar}><div style={s.pillRow}><Pill>{range.total} matching rows</Pill><Pill>{execution.mode}</Pill>{execution.lineupStatus ? <Pill tone="green">Lineups: {execution.lineupStatus}{execution.confirmedCount != null ? ` · ${execution.confirmedCount} confirmed` : ''}</Pill> : null}<Pill tone="green">Sorted by: {fieldMap[query.sort_by]?.label || titleCase(query.sort_by)} {query.sort_direction}</Pill></div><label style={s.pageSize}>Rows per page <select style={s.input} value={query.page_size} onChange={e => requestPageSize(e.target.value)}>{PAGE_SIZE_OPTIONS.map(size => <option key={size} value={size}>{size}</option>)}</select></label></div>{execution.ageHours != null && execution.ageHours > 36 ? <div style={s.warning}>Projection freshness warning: {execution.ageHours} hours old.</div> : null}{result?.filter_warnings?.length ? <div style={s.warning}>{result.filter_warnings.join(' • ')}</div> : null}{loading ? <StatePanel tone="loading" title="Loading report page">Loading the complete report.</StatePanel> : rows.length && visible.length ? <div style={s.dataGridWrap}><table style={s.table}><thead><tr>{visible.map(accessor => <th key={accessor} style={s.th}><button style={s.sortButton} disabled={fieldMap[accessor]?.sortable === false} onClick={() => requestSort(accessor)}>{fieldMap[accessor]?.label || titleCase(accessor)} {query.sort_by === accessor ? (query.sort_direction === 'desc' ? '↓' : '↑') : '↕'}</button></th>)}</tr></thead><tbody>{rows.map((row, rowIndex) => <tr key={row.entity_id || `${objectMeta.key}-${rowIndex}`}>{visible.map(accessor => <td style={s.td} key={`${rowIndex}-${accessor}`}>{formatCell(getValue(row, accessor))}</td>)}</tr>)}</tbody></table></div> : <StatePanel tone={bootstrapMessage.tone} title={bootstrapMessage.title}>{bootstrapMessage.detail}</StatePanel>}<div style={s.pagination}><button style={s.secondary} disabled={!pageInfo.has_previous || loading || exporting} onClick={() => requestPage(pageInfo.previous_page || Math.max(1, query.page_number - 1))}>← Previous</button><span>Page {pageInfo.page_number || 1} of {pageInfo.page_count || 1}</span><button style={s.secondary} disabled={!pageInfo.has_next || loading || exporting} onClick={() => requestPage(pageInfo.next_page || query.page_number + 1)}>Next →</button></div></main></div></section></div>
}
...
function MyDashboardReportBuilderContent() {
  ...
  const [results, setResults] = useState({}); const [loading, setLoading] = useState({}); const [error, setError] = useState(''); const [saveMessage, setSaveMessage] = useState(''); const [reportOpen, setReportOpen] = useState(false); const [reportObject, setReportObject] = useState('hitters'); const [reportResult, setReportResult] = useState(null); const [reportColumns, setReportColumns] = useState(DEFAULT_FIELDS); const [reportSavedItemId, setReportSavedItemId] = useState(null); const [generatedAt, setGeneratedAt] = useState(null); const [generatedForDate, setGeneratedForDate] = useState(reportDate); const [reportQuery, setReportQuery] = useState(defaultQueryState()); const [savedShelfOpen, setSavedShelfOpen] = useState(true); const [savedShelfView, setSavedShelfView] = useState('daily'); const [selectedShelfEntryKey, setSelectedShelfEntryKey] = useState(''); const [selectedFolderId, setSelectedFolderId] = useState(''); const [newFolderName, setNewFolderName] = useState(''); const [creatingFolder, setCreatingFolder] = useState(false); const [queryStudioRestore, setQueryStudioRestore] = useState(null)
  ...
  useEffect(() => { writeJson(BUILDER_KEY, { activeObject, reportDate, filters, selectedFieldsByObject, activeLineupsOnly, trendConfig }) }, [activeObject, reportDate, filters, selectedFieldsByObject, activeLineupsOnly, trendConfig])
  ...
  async function signOut() { await logoutDashboardSession().catch(() => null); setProfile(null); setWorkspace(null); setResults({}); setReportSavedItemId(null); setAuthMode('signin'); setAuthError('') }
  ...
  function setWeight(objectKey, metric, value) { setFilters(current => ({ ...current, [objectKey]: { ...(current[objectKey] || emptyFiltersForObject(objectKey)), weights: { ...(current[objectKey]?.weights || {}), [metric]: value } } })) }
  function invalidateGeneratedReports() { setResults({}); setReportOpen(false); setSaveAsOpen(false); setReportResult(null); setReportSavedItemId(null); setGeneratedAt(null); setError('') }
  ...
  function selectPrimaryObject(objectKey) { setActiveObject(objectKey); setResults(current => { const next = { ...current }; delete next[objectKey]; return next }); setReportOpen(false); setSaveAsOpen(false); setReportObject(objectKey); setReportResult(null); setReportSavedItemId(null); setReportColumns(defaultFieldsForObject(objectKey)); setReportQuery(defaultQueryState()); setError('') }
  function reportPageRequest(objectKey, queryOverride = null) {
    ...
    const request = buildReportRequest({ objectKey, activeLineupsOnly, date: reportDate, cleanedFilters: cleaned, query, selectedFields: selectableRequestFields(objectFields, availableFields), trendConfig: objectKey === 'player_trends' ? { ...trendConfig, window_days: Number(trendConfig.window_days), minimum_sample_size: Number(trendConfig.minimum_sample_size) } : undefined })
    return { objectFields, query, request }
  }
  async function fetchReportPage(prepared) {
    const raw = await apiJson(prepared.request.path, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(prepared.request.payload) })
    return prepared.request.reportType ? normalizeCanonicalPage(raw, prepared.query) : raw
  }
  async function populateReport(objectKey = activeObject, queryOverride = null, openWorkspace = true) { if (objectKey === 'player_trends') { const days = Number(trendConfig.window_days), minimum = Number(trendConfig.minimum_sample_size); if (!trendConfig.player_type || !trendConfig.comparison_baseline || !Number.isInteger(days) || days < 3 || days > 90 || !Number.isInteger(minimum) || minimum < 1 || !trendConfig.selected_metrics.length) { setError('Choose a player type, valid 3–90 day window, comparison baseline, minimum sample, and at least one trend metric.'); return null } } const prepared = reportPageRequest(objectKey, queryOverride); const { objectFields, query } = prepared; setLoading(current => ({ ...current, [objectKey]: true })); setError(''); setSaveMessage(''); try { const json = await fetchReportPage(prepared); setResults(current => ({ ...current, [objectKey]: json })); setReportObject(objectKey); setReportResult(json); setReportColumns(current => openWorkspace ? [...objectFields] : current); setReportQuery(query); setGeneratedAt(new Date().toISOString()); setGeneratedForDate(reportDate); if (openWorkspace) { setReportSavedItemId(null); setReportOpen(true) } return json } catch (err) { setError(err.message || 'Report generation failed'); return null } finally { setLoading(current => ({ ...current, [objectKey]: false })) } }
  function openSaved(item) {
    ...
    const columns = definition.selected_fields || payload.workbench_state?.selectedFields || payload.report_columns || selectedFieldsByObject[component] || defaultFieldsForObject(component)
    const sort = definition.sort || item?.sort_json || defaultQueryState()
    const savedDate = snapshot.generated_for_date || payload.saved_on_date || reportDate
    ...
    setFilters(current => ({ ...current, [component]: CANONICAL_REPORT_TYPES[component] ? normalizeSavedFilters(definition.filters || {}) : definition.filters || current[component] || emptyFilters() }))
    if (component === 'player_trends' && definition.trend_config) setTrendConfig({ ...EMPTY_TREND_CONFIG, ...definition.trend_config })
    setSelectedFieldsByObject(current => ({ ...current, [component]: [...columns] }))
    setResults(current => ({ ...current, [component]: board }))
    setReportResult(board)
    setReportSavedItemId(item.id)
    ...
    setGeneratedAt(snapshot.generated_at || item.created_at || new Date().toISOString())
    setReportOpen(true)
  }
  function saveReport() {
...
const s = {
  exportError: { marginTop: 7, color: C.red, fontSize: 11 },
