...
const tabs = ['Predictions', 'Games', 'Results & drivers', 'Model health']
...
function BookContext({ markets }) {
  ...
  return <div>{markets.slice(0, 2).map((market, index) => <small key={`${market.market}-${market.selection}-${index}`}>
    {market.book}: {market.selection} {fmt(market.line, 1)} ({market.price > 0 ? '+' : ''}{fmt(market.price, 0)}) · captured {new Date(market.captured_at).toLocaleString()} · line gap {market.line_gap > 0 ? '+' : ''}{fmt(market.line_gap)}
  </small>)}</div>
}

function Detail({ row, metric, onClose, baselineOnly }) {
  const prediction = row.predictions?.[metric] || {}
  const board = row.decision_board?.[metric] || {}
  const distribution = (!baselineOnly && prediction.distribution) || row.baseline_distributions?.[metric]
  return <section className="predicts-detail" aria-label={`${row.player_name} prediction details`}>
    ...
    <div className="predicts-summary-grid">
      ...
      {!baselineOnly && <><article><span>MLBGPT line</span><strong>{fmt(board.mlbgpt_line)}</strong><small>{board.line_authority === 'validated_residual_model' ? 'Validated residual adjustment' : 'Baseline retained—no proven adjustment yet'}</small></article>
    ...
    {!baselineOnly && <><h3>What drives this ranking</h3><p>Heuristic slate score, not a calibrated probability. Supporting signals can be correlated.</p>
...
export default function MLBGPTPredictsPage() {
  ...
  useEffect(() => {
    if (date !== getMlbToday()) return undefined
    const timer = setInterval(() => setRefreshKey(value => value + 1), 60000)
    return () => clearInterval(timer)
  }, [date])
  useEffect(() => {
    const controller = new AbortController()
    setLoading(true); setError(null); setSelected(null); setData(null); setAnalysis(null)
    ...
    }).then(body => { if (controller.signal.aborted) return; if (tab === 'Model health' || tab === 'Results & drivers') setAnalysis(body); else setData(body) })
  ...
  const rows = useMemo(() => (data?.records || []).filter(row => {
    const board = row.decision_board?.[metric]
    if (row.baseline?.[metric] == null) return false
    if (view === 'confirmed') return row.prediction_stage === 'confirmed_lineup' && (!shortlist || board?.category === 'confirmed_lineup_shortlist')
    return true
  }).sort((a, b) => view === 'baseline' ? b.baseline[metric] - a.baseline[metric] : (b.decision_board?.[metric]?.convergence_score || 0) - (a.decision_board?.[metric]?.convergence_score || 0)), [data, view, shortlist, metric])
  ...
  return <main className="predicts-page">
    <header className="predicts-hero"><div><p className="predicts-kicker">MLBGPT PREDICTION DESK</p><h1>From baseline to confirmed call</h1><p>Model Projections drives the early board. Confirmed lineups enable a separate evidence ranking. Numerical adjustments require holdout improvement; confirmation alone is not proof of accuracy.</p></div><label>Game date<input type="date" value={date} onChange={event => event.target.value && setDate(event.target.value)} /></label></header>
    <section className="predicts-pipeline" aria-label="Prediction timing"><div><span>1</span><strong>Model Projection</strong><small>Always the baseline</small></div><b>→</b><div><span>2</span><strong>Lineup confirmed</strong><small>Confirmed batting order</small></div><b>→</b><div><span>3</span><strong>MLBGPT funnel</strong><small>Trends + matchup + context</small></div><b>→</b><div><span>4</span><strong>Final & learn</strong><small>Grade every frozen call</small></div></section>
    <button type="button" disabled={loading} onClick={() => setRefreshKey(value => value + 1)}>Refresh saved predictions</button>
    ...
    {!['Games', 'Results & drivers', 'Model health'].includes(tab) && <div className="predicts-toolbar"><label>Prediction output<select value={view} onChange={event => { setView(event.target.value); setSelected(null) }}><option value="baseline">Model Projections — baseline</option><option value="confirmed">Confirmed Lineup Analysis</option></select></label>{view === 'confirmed' && <label><input type="checkbox" checked={shortlist} onChange={event => { setShortlist(event.target.checked); setSelected(null) }} />Shortlist only</label>}<label>Outcome<select value={metric} onChange={event => { setMetric(event.target.value); setSelected(null) }}>{Object.entries(labels).map(([key, value]) => <option key={key} value={key}>{value}</option>)}</select></label><p><strong>{counts.confirmed_lineup || 0}</strong> confirmed · <strong>{counts.model_projection_baseline || 0}</strong> awaiting lineup</p></div>}
    {tab === 'Predictions' && <p className="predicts-muted">{view === 'baseline' ? 'Original Model Projections values for every captured player, including confirmed players. Sorted by projected outcome.' : 'Confirmed players ranked using available frozen evidence. Missing inputs are not treated as positive signals.'} Today’s saved snapshots refresh on this page every minute. Times are shown in your device timezone.</p>}
    {loading && <p role="status">Loading prediction desk…</p>}{error && <p role="alert">{error}</p>}
    {!loading && !error && tab === 'Model health' && analysis && <section><h2>Residual model health</h2><p>Last capture: {analysis.last_capture ? new Date(analysis.last_capture).toLocaleString() : 'No snapshots yet'}</p><p>Latest refresh: {analysis.today_refresh?.status || 'No refresh recorded'}</p><p>{analysis.cold_start ? 'The learned adjustment is collecting enough real frozen predictions and Finals to prove it can beat Model Projections.' : 'Stored model runs are available. Adjustment readiness is checked separately for each metric and baseline version.'}</p><p className="predicts-muted">Until that proof exists, the MLBGPT numerical line remains the Model Projections baseline. The convergence board still ranks the strongest multi-source cases.</p></section>}
    {!loading && !error && tab === 'Results & drivers' && analysis && <section><h2>Results against Model Projections</h2><p>{analysis.record_definition}</p>{analysis.decision_performance?.length ? <div className="predicts-table-wrap"><table><thead><tr><th>Prediction</th><th>Outcome</th><th>Above / Below / Equal</th><th>Above baseline</th><th>Avg vs baseline</th></tr></thead><tbody>{analysis.decision_performance.map(row => <tr key={`${row.player_type}-${row.metric}-${row.category}-${row.method_version}`}><td>{driverLabel(row.category)}<small>{row.method_version}</small></td><td>{row.player_type} · {labels[row.metric]}</td><td>{row.above} / {row.below} / {row.equal}</td><td>{percent(row.win_rate)}</td><td>{row.average_actual_minus_baseline > 0 ? '+' : ''}{fmt(row.average_actual_minus_baseline)}</td></tr>)}</tbody></table></div> : <p>No graded predictions for this date yet.</p>}{analysis.winning_drivers?.length > 0 && <><h3>Signals present in frozen shortlists</h3><p>Observed associations; this does not establish which signal caused success.</p><div className="predicts-driver-cards">{analysis.winning_drivers.map(row => <article key={`${row.player_type}-${row.metric}-${row.driver}`}><span>{driverLabel(row.driver)}</span><strong>{percent(row.win_rate)}</strong><small>{row.wins}/{row.n} finished above baseline</small></article>)}</div></>}</section>}
    {!loading && !error && tab === 'Games' && <div className="predicts-game-grid">{games.map(row => <article key={row.game_pk}><span className="predicts-stage">{row.locked ? 'Pregame snapshot locked' : 'Pregame snapshots'}</span><h3>{row.team} vs {row.opponent}</h3><p>{new Date(row.game_time).toLocaleString()}</p><Link to={`/matchup/${row.game_pk}`}>Open matchup</Link>{row.outcome && <p><Link to={`/final/${row.game_pk}`}>View Final</Link></p>}</article>)}</div>}
    {!loading && !error && !['Games', 'Results & drivers', 'Model health'].includes(tab) && <>{rows.length ? <div className="predicts-table-wrap"><table><thead><tr><th>Rank</th><th>Player</th><th>Matchup</th><th>Stage</th><th>Model Projection</th>{view === 'confirmed' && <><th>MLBGPT line</th><th>Heuristic score</th><th>Signals</th><th>Book context</th></>}</tr></thead><tbody>{rows.map((row, index) => { const board = row.decision_board?.[metric] || {}; return <tr key={`${row.game_pk}-${row.player_id}-${row.player_type}`}><td className="predicts-rank">{index + 1}</td><td><button className="predicts-player" onClick={() => setSelected(row)}>{row.player_name || `Player ${row.player_id}`}</button><small>{row.player_type === 'batter' && row.batting_order ? `Batting ${row.batting_order}` : row.player_type}</small></td><td>{row.team} vs {row.opponent}<small>{new Date(row.game_time).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</small></td><td><span className={`predicts-stage ${row.prediction_stage}`}>{row.prediction_stage === 'confirmed_lineup' ? 'Confirmed' : 'Baseline'}</span></td><td>{fmt(row.baseline[metric])}</td>{view === 'confirmed' && <><td><strong>{fmt(board.mlbgpt_line)}</strong><small>{board.line_authority === 'validated_residual_model' ? 'Adjusted' : 'Baseline retained'}</small></td><td><span className="predicts-score">{fmt(board.convergence_score, 0)}</span></td><td>{board.supporting_signals} supporting<small>{board.opposing_signals} opposing</small></td><td><BookContext markets={board.book_markets} /></td></>}</tr> })}</tbody></table></div> : <p>{view === 'confirmed' ? (shortlist ? 'No confirmed players meet the shortlist criteria for this outcome.' : 'No confirmed-lineup analysis for this outcome yet. Select Model Projections to see the baseline.') : data?.reason || 'No eligible predictions.'}</p>}</>}
    {selected && <Detail row={selected} metric={metric} baselineOnly={view === 'baseline'} onClose={() => setSelected(null)} />}
  </main>
}
